#!/usr/bin/env python3
"""Train Track A Read/Write Sidecar on Qwen3.5-0.8B-Base.

Architecture:
  - Frozen Qwen3.5-0.8B-Base + LoRA adapters (Q/V projections, rank 16)
  - ESN reservoir (10K nodes) processes input embeddings + write signals
  - ReadProjection + CrossAttentionSidecar inject reservoir memory at selected layers
  - WriteHead: LLM hidden states → reservoir input signal (gated curriculum)

Starts from T14 checkpoint (read-only pretrained).
Gated write: gate linearly increases 0→1 over first 1K steps (write_gate_warmup_steps).

Usage::

    python scripts/train_track_a_readwrite.py
    python scripts/train_track_a_readwrite.py --checkpoint checkpoints/track_a_readonly/final \\
        --max_steps 5000 --output_dir checkpoints/track_a_readwrite
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train Track A Read/Write Sidecar")
    p.add_argument("--model_name", default="qwen3.5-0.8b")
    p.add_argument("--dtype", default="bfloat16", choices=["float16", "bfloat16"])
    p.add_argument("--device", default="cuda")

    # Checkpoint from T14 (read-only pretrained)
    p.add_argument(
        "--checkpoint",
        default=None,
        help="Path to T14 checkpoint dir containing lora_adapter/ and sidecar_weights.pt",
    )
    ...
    p.add_argument(
        "--write_gate_warmup_steps",
        type=int,
        default=1000,
        help="Steps over which write gate linearly increases from 0 to 1.",
    )
    ...
    # Output
    p.add_argument("--output_dir", default="checkpoints/track_a_readwrite")
    p.add_argument("--results_file", default="results/track_a/readwrite.json")
    p.add_argument("--log_interval", type=int, default=50)
    p.add_argument("--save_interval", type=int, default=1000)
    p.add_argument("--no_wandb", action="store_true")
    p.add_argument("--wandb_project", default="lrs-track-a")
    p.add_argument("--wandb_run_name", default="track-a-readwrite-qwen3.5-0.8b")
...
# ---------------------------------------------------------------------------
# Read/Write sidecar module (with WriteHead)
# ---------------------------------------------------------------------------


class ReadWriteSidecarBundle(nn.Module):
    """Bundle of ReadProjection + CrossAttentionSidecar + WriteHead modules.

    Extends ReadOnlySidecarBundle by adding a WriteHead that projects LLM
    hidden states back to the reservoir input dimension.  The write gate
    (a scalar in [0, 1]) is applied externally to implement the curriculum.

    Architecture:
      - CrossAttentionSidecar per injected layer: reservoir K/V × LLM hidden Q
      - ReadProjection: reservoir state → LLM hidden dim (shared)
      - WriteHead: LLM hidden state → reservoir state dim (gated externally)
    """
    ...
    def __init__(
        ...
        from src.reservoir.interface import CrossAttentionSidecar, ReadProjection, WriteHead
        ...
        # WriteHead: projects LLM hidden states → reservoir state dim (for ESN w_t drive)
        self.write_head = WriteHead(
            hidden_dim=hidden_dim,
            input_dim=reservoir_dim,
        )
...
def compute_reservoir_states_readonly(
    esn: object,
    embeddings: torch.Tensor,
) -> np.ndarray:
    """Run input embeddings through ESN (read-only: no write signal).

    Args:
        esn: ESN instance.
        embeddings: Input embeddings, shape (batch, seq_len, embed_dim).

    Returns:
        Reservoir states, shape (batch, seq_len, reservoir_dim).
    """
    B, T, _ = embeddings.shape
    emb_np = embeddings.detach().float().cpu().numpy()  # (B, T, H)
    states = np.zeros((B, T, esn.n), dtype=np.float32)  # type: ignore[attr-defined]

    for b in range(B):
        esn.reset()  # type: ignore[attr-defined]
        for t in range(T):
            states[b, t] = esn.step(emb_np[b, t])  # type: ignore[attr-defined]

    return states


def compute_reservoir_states_readwrite(
    esn: object,
    embeddings: torch.Tensor,
    write_signals: np.ndarray,
    write_gate: float,
) -> np.ndarray:
    """Run input embeddings + gated write signals through ESN.

    Write signals are delayed by one position (w_t drives step t+1) to avoid
    circular dependency: the LLM's hidden state at position t is used as the
    write signal for the reservoir update at position t+1.

    Args:
        esn: ESN instance.
        embeddings: Input embeddings, shape (batch, seq_len, embed_dim).
        write_signals: Write signals from WriteHead, shape (batch, seq_len, reservoir_dim).
        write_gate: Scalar in [0, 1]; gate=0 is pure read-only, gate=1 is full write.
    ...
    for b in range(B):
        ...
        for t in range(T):
            x_t = emb_np[b, t]
            # Delayed write: use write signal from previous position (t-1)
            w_t = write_gate * write_signals[b, t - 1] if t > 0 else None
            states[b, t] = esn.step(x_t, w_t)  # type: ignore[attr-defined]
...
class SidecarHookManager:
    """Registers and manages forward hooks for read/write sidecar injection.
    ...
    def __init__(
        self,
        model: nn.Module,
        sidecar_bundle: ReadWriteSidecarBundle,
        layer_indices: list[int],
    ) -> None:
...
class HiddenStateCaptureHook:
    """Captures hidden states from a specific transformer layer.

    Used to obtain LLM hidden states for write signal computation.
    """

    def __init__(self, model: nn.Module, layer_idx: int) -> None:
        self.captured: torch.Tensor | None = None
        transformer_layers = SidecarHookManager._get_transformer_layers(model)
        if layer_idx < len(transformer_layers):
            self._handle = transformer_layers[layer_idx].register_forward_hook(
                self._hook
            )
        else:
            logger.warning("Capture layer %d out of range; write signals disabled.", layer_idx)
            self._handle = None

    def _hook(self, module: nn.Module, inputs: tuple, output: Any) -> None:
        if isinstance(output, tuple):
...
def load_t14_checkpoint(
    checkpoint_dir: str,
    model: nn.Module,
    sidecar_bundle: ReadWriteSidecarBundle,
    device: torch.device,
) -> None:
    """Load T14 (read-only) checkpoint weights into model and sidecar.

    Loads LoRA adapter weights (if present) and sidecar weights (ReadProjection
    + CrossAttentionSidecar).  WriteHead is left randomly initialized since it
    was not trained in T14.
    """
    ckpt = Path(checkpoint_dir)
    lora_dir = ckpt / "lora_adapter"
    sidecar_file = ckpt / "sidecar_weights.pt"

    if lora_dir.exists() and hasattr(model, "load_adapter"):
        logger.info("Loading T14 LoRA adapter from %s", lora_dir)
        try:
            model.load_adapter(str(lora_dir))
        except Exception as exc:
            logger.warning("LoRA load failed: %s — continuing without.", exc)
    else:
        logger.info("No lora_adapter dir found at %s — skipping LoRA load.", lora_dir)

    if sidecar_file.exists():
        logger.info("Loading T14 sidecar weights from %s", sidecar_file)
        state_dict = torch.load(sidecar_file, map_location=device)
        # Load only matching keys (sidecars + read_proj); WriteHead is new
        missing, unexpected = sidecar_bundle.load_state_dict(state_dict, strict=False)
        logger.info(
            "Sidecar weights loaded: %d missing keys (expected: write_head.*), "
            "%d unexpected keys.",
            len(missing),
            len(unexpected),
        )
    else:
        logger.info("No sidecar_weights.pt at %s — starting from random.", sidecar_file)
...
def train(args: argparse.Namespace) -> None:
    ...
    logger.info(
        "ReadWriteSidecarBundle: %d layers, reservoir_dim=%d, hidden_dim=%d",
        len(sidecar_layers), args.reservoir_size, hidden_dim,
    )

    # --- Load T14 checkpoint (optional) ---
    if args.checkpoint:
        load_t14_checkpoint(args.checkpoint, model, sidecar_bundle, device)
    else:
        logger.info("No --checkpoint provided; training from scratch (no T14 pretraining).")

    # --- Register forward hooks ---
    hook_manager = SidecarHookManager(model, sidecar_bundle, sidecar_layers)

    # Capture layer for write signal: use the first sidecar layer
    capture_layer_idx = sidecar_layers[0] if sidecar_layers else 0
    ...
    # --- Optimizer ---
    # LoRA params at lr; sidecar params (including WriteHead) at interface_lr
    ...
    for batch in loader:
        ...
        # Write gate: linear warmup 0 → 1 over write_gate_warmup_steps
        write_gate = min(1.0, global_step / max(1, args.write_gate_warmup_steps))

        input_ids = batch["input_ids"].to(device)
        labels = batch["labels"].to(device)

        # --- Compute input embeddings (for ESN) ---
        with torch.no_grad():
            embeddings = embed_layer(input_ids)  # (B, T, H)

        # --- Compute reservoir states ---
        if write_gate > 0:
            # Two-pass approach: first pass (no-grad) to get hidden states,
            # then compute write signals, then re-compute reservoir states,
            # then do the actual training pass.

            # Pass 1: compute read-only reservoir states
            init_states = compute_reservoir_states_readonly(esn, embeddings)

            # Capture hidden states from first sidecar layer
            capture_hook = HiddenStateCaptureHook(model, capture_layer_idx)
            hook_manager.set_reservoir_states(init_states)
            with torch.no_grad():
            ...
            # Compute write signals using WriteHead
            if capture_hook.captured is not None:
                h_capture = capture_hook.captured  # (B, T, H)
                write_signals = sidecar_bundle.write_head.to_numpy(
                    h_capture.float()
                )  # (B, T, reservoir_dim)
                write_norm = float(np.linalg.norm(write_signals))
            else:
                write_signals = np.zeros(
                    (embeddings.shape[0], embeddings.shape[1], args.reservoir_size),
                    dtype=np.float32,
                )
                write_norm = 0.0

            # Re-compute reservoir states with write signals
            reservoir_states = compute_reservoir_states_readwrite(
                esn, embeddings, write_signals, write_gate
            )
        else:
            # gate=0: pure read-only (faster, no second pass needed)
            reservoir_states = compute_reservoir_states_readonly(esn, embeddings)
            write_norm = 0.0

        write_norms.append(write_norm)

        # --- Training forward pass (hooks inject reservoir states) ---
        ...
        if global_step % args.log_interval == 0:
            ...
            avg_write_norm = float(np.mean(write_norms[-args.log_interval:]))
            logger.info(
                "step=%d  loss=%.4f  write_gate=%.3f  write_norm=%.4f  "
                "lr=%.2e  elapsed=%.0fs",
                global_step,
                avg_loss,
                write_gate,
            ...
            try:
                ...
                wandb.log({
                    ...
                    "write/signal_norm": avg_write_norm,
    ...
    final_write_norm = float(np.mean(write_norms[-100:])) if write_norms else 0.0
    # Compute entropy of recent write signals (as proxy for non-triviality)
    write_nontrivial = final_write_norm > 1e-6
    ...
    results = {
        ...
        "architecture": {
            ...
            "write_head": True,
            "write_gate_warmup_steps": args.write_gate_warmup_steps,
        },
        "training": {
            ...
            "t14_checkpoint": args.checkpoint,
        },
        "metrics": {
            "final_train_loss": final_loss,
            "final_train_perplexity": final_ppl,
            "loss_history": losses[::max(1, len(losses) // 100)],
            "write_norm_history": write_norms[::max(1, len(write_norms) // 100)],
            "final_write_norm": final_write_norm,
            "write_head_nontrivial": write_nontrivial,
        },
    ...
    logger.info(
        "Done.  final_loss=%.4f  final_ppl=%.2f  write_nontrivial=%s  steps=%d",
        final_loss, final_ppl, write_nontrivial, global_step,
    )
